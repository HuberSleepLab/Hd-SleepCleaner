classdef visfun
    methods(Static)
        ...
        function [trackNdxGood] = trackvis(vistrack)
        
            % *************************************************************
            % Function to find clean epochs accoring to vistrack in out old
            % sleepp scoring programm (the ticks on top)
            %
            % *** Input ***
            % #vistrack         Track as from readtrac()
            %
            % *** Output ***
            % #trackNdxGood     Good 20s epochs
            % *************************************************************
            
            trackSum     = sum(vistrack == 49, 3); % summarize all tracks, 49: bad epoch, number indicates how many tracks were bad in a 4s epoch.
            trackSum     = sum(trackSum,2);        % summarize all 4s windows per 20s window, >0: bad epoch 
            trackNdxGood = trackSum'==0;     % the zero's are good 20s epochs, find them           
